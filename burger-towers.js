...
export class BurgerTowers extends Scene {
  ...
  constructor() {
    ...
    this.model_transform_box_1 = Mat4.identity().times(
      Mat4.translation(-2, 0, 0)
    );
    this.model_transform_box_2 = Mat4.identity().times(
      Mat4.translation(2, 0, 0)
    );
  ...
  display(context, program_state) {
    ...
    if (this.isSpinning) {
      const box_1_rotation = (dt / 3) * Math.PI;
      this.model_transform_box_1 = this.model_transform_box_1.times(
        Mat4.rotation(box_1_rotation, 1, 0, 0)
      );
      const box_2_rotation = (dt / 2) * Math.PI;
      this.model_transform_box_2 = this.model_transform_box_2.times(
        Mat4.rotation(box_2_rotation, 0, 1, 0)
      );
    }
    ...
    this.shapes.box_1.draw(
      context,
      program_state,
      this.model_transform_box_1,
      this.materials.texture_stars
    );
    this.shapes.box_2.draw(
      context,
      program_state,
      this.model_transform_box_2,
    ...
    return (
      ...
      `
        ...
        void main(){
            ...
            // the center of each face at a rate of 20 rpm.
            // Prevent the rotation angle from growing excessively 
            // as animation_time grows: 

            // 20 rpm --> (1/3) PI rad/s
            float rot_angle = 0.333 * 3.14159265 * mod(animation_time, 4.); 
